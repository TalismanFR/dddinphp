...
Объекты-значения являются фундаментальным строительным блоком `Domain-Driven Design`, и они используются для моделирования
понятий вашего `Универсального Языка` в коде.  
`Объект-значение` - это не просто вещь в вашей `Доменной модели` - он измеряет, количественно оценивает или описывает что-то.
`Объекты-значения` можно рассматривать как маленькие, простые объекты, такие как деньги или диапазон дат, равенство
которых определяется не на основе идентичности, а на содержимом, которое они несут.

Например, цену продукта можно моделировать с помощью объекта-значения. В данном случае он не представляет собой
конкретную вещь, а скорее значение, позволяющее нам измерить стоимость продукта в деньгах.
Потребление памяти для этих объектов легко определить (рассчитывается на основе их составных частей), и здесь почти нет
накладных расходов. В результате предпочтительным является создание новых экземпляров перед повторным использованием
ссылок, даже когда они используются для представления одного и того же значения.
Равенство затем проверяется на основе сравнимости полей обоих экземпляров.
...
Примерами объектов-значений являются числа, текстовые строки, даты, времена, полное имя человека
(состоящее из имени, отчества, фамилии и титула), валюты, цвета, номера телефонов и почтовые адреса.
...
Рассмотрим следующие примеры из [Википедии](http://en.wikipedia.org/wiki/Domain-driven_design#Building_blocks_of_DDD),
чтобы лучше понять разницу между объектами-значениями и сущностями:  
...
Наконец, ваша `Сущность` будет занимать в базе ровно столько места, сколько ей требуется - не больше и не меньше.
...
с поддержкой сопоставления Объектов Значений, что устраняет необходимость делать это
самостоятельно, как в Doctrine 2.4. С декабря 2015, Doctrine также поддерживает вложенность. Поддержка не 
100 процентная, но этого вполне хватит чтобы попробовать. В случае, если это не подходит для вашего сценария,
обратитесь к следующему разделу. Официальную документацию можно найти в справочнике Doctrine Embeddables. Этот вариант, 
если он будет реализован правильно, мы рекомендуем больше всего.
Это наиболее простое и элегантное решение, которое также обеспечивает поддержку поиска в ваших DQL-запросах.

Поскольку классы `Product`, `Money` и `Currency` умы уже показывали, остается только показать файл настройки
...
В отображении товара мы определяем цену как переменную объекта класса `Money`.
В то же время, `Money` конструируется с помощью значения `amount` и объекта класса `Currency`:
...
```                  
Наконец, пришло время показать отображение Doctrine для нашего Объекта Значение класса `Currency`:
...
Как видите, приведенный выше код имеет стандартное определение вложенности с одним строковым полем,
содержащим код `ISO`. Этот подход является самым простым способом использования встраиваемых объектов и 
гораздо эффективнее. По умолчанию, `Doctrine` присваивает имена вашим столбцам, добавляя к ним префикс, используя имя
...
## Embedded Value with Doctrine <= 2.4.* (Вложенные значения с Doctrine <= 2.4.*)

Если вы до сих пор используете `Doctrine 2.4`, то вам может быть интересно приемлемое решение для использования
`Embedded Values` в версии `Doctrine` < 2.5. Мы теперь должны заместить все атрибуты объекта-значения в сущности
`Продукт`, что означает создание новых искусственных атрибутов, которые будут содержать информацию об объекте-значении.
После этого мы сможем отобразить все эти новые атрибуты с использованием `Doctrine`. Давайте посмотрим, как это повлияет
на сущность "Продукт".
``` php
class Product
{
...
Как видите, появились два новых атрибута: один для суммы и другой для `ISO`-кода валюты. Мы также обновили метод `setPrice`
для поддержания согласованности атрибута при его установке. Кроме того, мы обновили геттер цены, чтобы возвращать
объект-значение `Money`, построенный из новых полей. Давайте посмотрим, как должно измениться соответствующее
`XML`-отображение `Doctrine`:
``` XML
<?xml version="1.0" encoding="utf-8"?>
...
> #### Суррогатные (замещающие) атрибуты
>
> Эти два новых поля строго не относятся к `Домену`, так как они не касаются деталей `Инфраструктуры`.
> Скорее, они являются необходимостью из-за отсутствия поддержки встраиваемых объектов в `Doctrine`.
> Существуют альтернативы, которые могли бы исключить эти два атрибута из чистого `Домена`; однако этот подход более простой,
> удобный и, как компромисс, приемлемый. Еще один пример использования замещающих атрибутов в этой книге можно найти 
> в подразделе `Замещающая идентичность` раздела `Операции с идентичностью` главы 4, `Сущности`.

Если бы мы хотели исключить эти два атрибута из `Домена`, этого можно было бы достичь
с помощью использования `Абстрактной Фабрики`.
Сначала нам нужно создать новую сущность, `DoctrineProduct`, в нашей директории `Инфраструктура`. Эта сущность будет
наследовать сущность `Product`. Все замещающие поля будут помещены в новый класс, и методы, такие как `price` или
`setPrice`, должны быть реализованы заново.
Мы настроим `Doctrine` использовать новый `DoctrineProduct` вместо сущности `Product`.

Теперь мы можем извлекать сущности из базы данных, но как насчет создания нового `Продукта`?
В какой-то момент нам потребуется вызвать `new Product()`, но поскольку нам нужно иметь дело с `DoctrineProduct`, и мы
не хотим, чтобы наши `Сервисы Приложения` знали о деталях `Инфраструктуры`, нам потребуется использовать `Фабрики` для
создания `Сущностей` `Продуктов`. Таким образом, в каждом случае, где происходит создание `Сущности` с использованием
`new`, вы вместо этого будете вызывать `createProduct` у `ProductFactory`.

Могут потребоваться множественные дополнительные классы, чтобы избежать помещения замещающих атрибутов
в исходную `Сущность`. По этой причине нашей рекомендацией является замещение всех объектов-значений в одной и той
же `Сущности`, хотя это, безусловно, приводит к менее чистому решению.

## Serialized LOB и специальная ORM
Если добавление возможности поиска по атрибутам объектов-значений не является важным, можно рассмотреть ещё один шаблон:
`Сериализованный LOB` (`Large Object`). Этот шаблон работает путем сериализации всего объекта-значения в строковый формат,
который можно легко сохранять и извлекать. Самое существенное различие между этим решением и альтернативой с
встраиванием заключается в том, что во втором варианте требования к сохраняемому пространству сведены к одному столбцу.
...
Для сохранения `Сущностей` `Продукт` с использованием данного подхода требуется изменение `DbalProductRepository`.
Объект-значение `Money` должен быть сериализован в строку перед сохранением конечной `Сущности`.
...
Тип столбца `price` `TEXT`, в ней хранится сериализованный объект `Money`, представлющий `9.99 USD`
...
Этот подход выполняет свою функцию. Тем не менее, он не рекомендуется из-за проблем, возникающих при рефакторинге
классов в вашем коде.  
Можете ли вы представить проблемы, если бы мы решили переименовать наш класс `Money`?  
Можете ли вы представить изменения, которые были бы необходимы в нашем представлении базы данных, когда класс `Money`
перемещается из одного пространства имен в другое?  
Еще один компромисс, как уже объяснялось, заключается в отсутствии возможности выполнения запросов.  
Неважно, используете ли вы `Doctrine` или нет; написание запроса для получения продуктов дешевле, скажем, 200 долларов
США, практически невозможно при использовании стратегии сериализации.

Проблему с запросами можно решить только с использованием `Встраиваемых Значений` (`Embedded Values`).
Тем не менее, проблемы с рефакторингом сериализации можно решить с помощью специализированной библиотеки для
обработки процессов сериализации.

### Улучшенная сериализация с помощью JMS Serialize

Стандартные стратегии сериализации/десериализации в `PHP` имеют проблему при работе с рефакторингом классов и
пространств имен. Одной из альтернатив является использование собственного механизма сериализации, например,
объединения суммы, разделителя вроде одного символа (например, `|`), и кода `ISO` валюты. Тем не менее, есть еще один
предпочтительный подход: использование библиотеки сериализации с открытым исходным кодом, такой как `JMS Serializer`.
Давайте рассмотрим пример его применения при сериализации объекта `Money`:  
...
С помощью этого примера вы можете провести рефакторинг вашего класса `Money`, не обновляя при этом базу данных.
`JMS Serializer` может быть использован во многих различных сценариях, например, при работе с `REST API`.
Важной особенностью является возможность указать, какие атрибуты объекта следует исключить из процесса сериализации,
например, пароль.

Для получения дополнительной информации ознакомьтесь с `Руководством по Маппингу` и `Книгой рецептов` (`Cookbook`).
`JMS Serializer` является неотъемлемым инструментом в любом проекте, основанном на архитектуре `Domain-Driven Design`.
...
`Doctrine` поддерживает шаблон `Serialize LOB`. Существует множество предопределенных типов отображения,
которые можно использовать для сопоставления атрибутов `Сущности` со столбцами базы данных или даже таблицами.
Одним из этих отображений является тип `object`, который сопоставляет `SQL CLOB` с объектом `PHP`,
используя функции `serialize()` и `unserialize()`.
...
>- Отображает и преобразует данные объекта на основе сериализации в PHP.
>Если вам необходимо сохранить точное представление данных вашего объекта, рекомендуется использовать этот тип, так как
>он использует сериализацию для представления точной копии вашего объекта в виде строки в базе данных.
>Значения, извлеченные из базы данных, всегда преобразуются в тип объекта PHP с использованием десериализации или 
>в значение `null`, если данных нет.
>
...
>
>- Поскольку встроенный тип `text` в `PostgreSql` не поддерживает `NULL`-байты, при десирализации будет возникать ошибка.
...
1 row in set (0.00 sec)
```
Этот подход явно лучше про сравнению с предыдущим с точки зрения будущего рефакторинга.
